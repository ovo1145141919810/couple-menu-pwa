...
代码已经包含正式模式、数据库迁移、双账号权限、私有照片、实时同步和自动部署。下面是仍需项目维护者在外部平台完成的一次性配置。女朋友不需要 GitHub、Supabase 或编程软件，只需要最终网址与她的应用账号。
...
迁移会创建七张业务表、原子状态函数、RLS、Realtime 发布配置，以及私有的 `dish-images`、`interaction-icons` Bucket。
...
where schemaname = 'public'
  and tablename in ('profiles', 'categories', 'dishes', 'interaction_options', 'wishlists', 'wishlist_items', 'reviews')
order by tablename;
...
第一条查询的七行 `rowsecurity` 都必须是 `true`；第二条应包含菜单、心愿与评价相关表。
...
- 上传失败：确认四个迁移都已执行，并检查两个 Storage Bucket 与策略。
