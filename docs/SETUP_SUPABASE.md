...
8. `202607200008_interaction_message_replies.sql`
9. `202607200009_interaction_categories_and_layout.sql`

迁移会创建九张受 RLS 保护的表、原子状态函数、Realtime 发布配置，以及私有的 `dish-images`、`interaction-icons` Bucket。
...
7. 双方分别创建、改名互动分类，并把预设或自定义互动拖到不同分类后保存，确认另一台设备实时同步相同排版。
8. 退出登录后，真实业务请求应无法读取数据。
9. 将应用切到后台再回来，数据应自动刷新；购物车在短暂断网时仍保留。
10. 两台手机分别从桌面 PWA 打开“消息提醒”，点击开启，再点击“发送测试通知”；关闭应用后互发一次互动，验证系统通知。
...
- 上传失败：确认九个迁移都已执行，并检查两个 Storage Bucket 与策略。
- 收不到后台推送：确认已从主屏幕 PWA 打开并允许通知，进入“消息提醒”点击“发送测试通知”以重新同步订阅；仍无通知时检查 iPhone 的通知权限、专注模式、Edge Function 与三项 VAPID Secrets。
- 没有实时变化：确认 `supabase_realtime` publication 包含七张变化表，重新打开应用会主动刷新。
