...
- 双方：发起自定义互动，可上传私人图片图标；共同创建互动分类，并用触屏拖动自定义分类与排序；兑现后可用一句话或另一个互动回礼。
- 留言：无论是撒娇拒绝的留言，还是兑现后的文字回礼，原发起人都可以再回复一句，保留完整的两轮对话。
- 氛围：温柔奶油粉纯色背景、逐字页面标题和按钮点击火花。
- 回忆：已上菜、已兑现互动与评价自动组成时间线。
- 待处理：等我回应、我发出的心愿和我回应过的记录使用三段折叠导航，减少长列表滚动。
- 作品集 Demo：一键切换双角色，使用独立假数据，完全不访问生产数据。
- PWA：手机优先、可添加到桌面、前台实时同步，并支持网页关闭后的系统消息推送；心愿、状态、文字回复、新菜和新互动都会提醒对方。
